...
if ! command -v rustup >/dev/null 2>&1; then
  read -r -p "  rustup is not installed. Install it now from https://rustup.rs? [Y/n] " answer
  if [[ "${answer:-Y}" =~ ^[Nn] ]]; then
...
if [[ -f "$LOCAL_XCCONFIG" ]]; then
  ...
  echo
  echo "  Your Team ID is the 10-character code shown in Xcode > Settings > Accounts,"
  echo "  or the OU field of your 'Apple Development' certificate."
  read -r -p "  Team ID (leave blank to pick the team in Xcode instead): " team_id
